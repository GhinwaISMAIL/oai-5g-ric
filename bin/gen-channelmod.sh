...
#
# Usage: gen-channelmod.sh <ues_per_cell> <out_file> [mode] [type] [tdl_delay_spread_ns]
...
MODE="${3:-uniform}"
TYPE="${4:-AWGN}"
TDL_DELAY_SPREAD_NS="${5:-30}"

case "${TYPE}" in
    TDL_A|TDL_B|TDL_C)
        case "${TDL_DELAY_SPREAD_NS}" in
            10)  DS_TDL=0.00000001 ;;
            30)  DS_TDL=0.00000003 ;;
            100) DS_TDL=0.00000010 ;;
            *)
                echo "[CHANMOD] ERROR: TDL delay spread must be 10, 30, or 100 ns" >&2
                exit 2
                ;;
        esac
        ;;
...
{
  echo "# Generated by gen-channelmod.sh — ${UES} UEs, mode=${MODE}, type=${TYPE}, tdl_delay_spread_ns=${TDL_DELAY_SPREAD_NS}"
...
echo "[CHANMOD] wrote ${OUT}: 1 gNB model + ${UES} UE models (${MODE}, ${TYPE}, tdl_delay_spread_ns=${TDL_DELAY_SPREAD_NS})"
