#!/usr/bin/env python3
"""Run the frozen five-repetition static TDL-B scalar-transfer pilot.

The pilot replaces only UE1, recreates it with one explicit OAI RNG seed per
repetition, verifies the active downlink channel is TDL_B, replays the frozen
five-level scalar envelope, and restores the original UE in a ``finally``
...
PARSER_SPEC = importlib.util.spec_from_file_location("phase3c13_log_parser", PARSER_SCRIPT)
...
RNG_SEEDS = (31001, 31002, 31003, 31004, 31005)
ENVELOPE = BASE.ENVELOPE
SEGMENT_SECONDS = BASE.SEGMENT_SECONDS
SETTLING_SECONDS = BASE.SETTLING_SECONDS
FIXED_NOISE_DB = BASE.FIXED_NOISE_DB
DEBUG_IMAGE = "oai-nr-ue-phase3c13-static:70508eb"
ORIGINAL_IMAGE = "ghinwa555/oai-nr-ue-chan:v5"
...
def channel_command(
    ...
    if result.get("model_type") != CHANNEL_FAMILY:
        raise ReplayError(
            f"the Phase 3C13 pilot requires {CHANNEL_FAMILY}, observed: {result}"
        )
...
def run_one_replay(
    ...
    if BASE.docker_inspect(
        '{{range .Config.Env}}{{println .}}{{end}}', UE_CONTAINER
    ).splitlines().count(f"OAI_RNGSEED={rng_seed}") != 1:
    ...
    if len(fingerprints) != 1:
        raise ReplayError(f"{replay_id} did not retain exactly one tap fingerprint")
    ...
    dimensions = {
        (int(row["channel_length"]), int(row["nb_taps"])) for row in rows
    }
    if len(dimensions) != 1 or next(iter(dimensions))[0] <= 1:
        raise ReplayError(f"{replay_id} does not expose a multipath channel: {dimensions}")
    ...
    return rows, {
        ...
        "channel_dimensions": {
            "channel_length": next(iter(dimensions))[0],
            "nb_taps": next(iter(dimensions))[1],
        },
...
def make_output(root: Path) -> Path:
    ...
    output = root / f"phase3c13-static-tdlb-{stamp}"
...
def execute(args: argparse.Namespace) -> int:
    ...
    if BASE.image_id(args.debug_image) != args.expected_debug_image_id:
        raise ReplayError("pinned Phase 3C13 debug image ID is unavailable")
    ...
    if BASE.docker_inspect("{{.State.Health.Status}}", GNB_CONTAINER) != "healthy":
        raise ReplayError("gNB is not healthy before the pilot")
    ...
    try:
        ...
        if len(fingerprints) != len(RNG_SEEDS):
            raise ReplayError(
                "explicit independent RNG seeds did not produce distinct TDL-B fingerprints"
            )
        write_csv(output / "static_tdlb_scalar_telemetry.csv", all_rows)
    ...
    if error is not None:
        ...
        return 1
    print(f"TELEMETRY={output / 'static_tdlb_scalar_telemetry.csv'}", flush=True)
    return 0
...
def parser() -> argparse.ArgumentParser:
    ...
    root.add_argument(
        "--ue-config", default="/local/repository/etc/nr-ue-cell1-1.conf"
    )
    root.add_argument(
        "--channel-helper", default="/local/repository/bin/channel-cell.py"
    )
    root.add_argument(
        "--override-file", default="/local/phase3c/phase3c13-ue-debug.override.yaml"
    )
