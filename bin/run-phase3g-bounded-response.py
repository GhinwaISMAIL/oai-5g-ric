#!/usr/bin/env python3
"""Run the frozen one-UE AWGN gain/noise response experiment."""
...
SUPPORT_SPEC = importlib.util.spec_from_file_location(
    "phase3g_bounded_response_support", SUPPORT_SCRIPT
)
...
RESEARCH_REVISION = "95981478b2e142735aea666913669cbdb43fc514"
RESEARCH_PROTOCOL_SHA256 = (
    "deae9dd0991242c9ef585a6492d31014001bda1f600ca1549df36564b5a507b4"
)
EXPECTED_PLAN_SHA256 = (
    "63b8d0e8880c1ccc418923d10e375f43f3c71c99b80ff75b0fedb57cbc7e21e7"
)
CHANNEL_FAMILY = "AWGN"
DEBUG_IMAGE = SUPPORT.DEBUG_IMAGE
...
def load_execution_plan(path: Path) -> list[dict[str, Any]]:
    SUPPORT.require_hash(path, EXPECTED_PLAN_SHA256)
    with path.open(newline="") as stream:
        reader = csv.DictReader(stream)
        if tuple(reader.fieldnames or ()) != PLAN_FIELDS:
            raise ValidationError(
                f"unexpected response-plan columns: {reader.fieldnames}"
            )
    ...
    for index, row in enumerate(raw_rows, start=1):
        try:
            ...
            raise ValidationError(
                f"invalid response-plan row {index}: {row}"
            ) from error
        if not math.isfinite(parsed["gain_db"]) or not math.isfinite(
            parsed["noise_power_db"]
        ):
            raise ValidationError(f"non-finite response-plan row {index}")
        rows.append(parsed)

    counts = Counter(row["stage"] for row in rows)
    if len(rows) != 45 or counts != {
        "gain_safety": 4,
        "noise_safety": 2,
        "factorial": 27,
        "boundary": 12,
    }:
        raise ValidationError(f"unexpected bounded-response plan: {counts}")
    if [row["stage"] for row in rows[:6]] != ["gain_safety"] * 4 + ["noise_safety"] * 2:
        raise ValidationError("all safety executions must precede the crossed design")
    if len({row["oai_rng_seed"] for row in rows}) != len(rows):
        raise ValidationError("response-plan RNG seeds are not unique")
    factorial = Counter(
        (row["gain_db"], row["noise_power_db"])
        for row in rows
        if row["stage"] == "factorial"
    )
    boundary = Counter(
        (row["gain_db"], row["noise_power_db"])
        for row in rows
        if row["stage"] == "boundary"
    )
    if len(factorial) != 9 or set(factorial.values()) != {3}:
        raise ValidationError("factorial states are not repeated exactly three times")
    if len(boundary) != 4 or set(boundary.values()) != {3}:
        raise ValidationError("boundary states are not repeated exactly three times")
    return rows
...
def build_telemetry_rows(
    ...
    channel_rows = SUPPORT.SUPPORT.parse_marker_lines(
        log_text, "RFSIM_CHANNEL_DEBUG_V1"
    )
    required_channel = {
        "model",
        "channel_snapshot_id",
    ...
    for utc_second, ue in sorted(ue_rows.items()):
        ...
        if channel["oai_rng_seed"] != str(plan_row["oai_rng_seed"]):
            raise ValidationError(
                f"RNG seed mismatch: expected {plan_row['oai_rng_seed']}, "
                f"observed {channel['oai_rng_seed']}"
            )
        row = {
            "execution_index": plan_row["execution_index"],
            "execution_id": execution_id,
            "stage": plan_row["stage"],
            "stage_position": plan_row["stage_position"],
            "repetition": plan_row["repetition"],
            "oai_rng_seed": plan_row["oai_rng_seed"],
            "commanded_gain_db": plan_row["gain_db"],
            "applied_gain_db": channel["applied_gain_db"],
            "commanded_noise_power_db": plan_row["noise_power_db"],
        ...
        numeric_fields = set(TELEMETRY_FIELDS) - string_fields
...
def run_one_execution(
    ...
    *,
    ...
    execution_id = (
        f"e{plan_row['execution_index']:02d}-{plan_row['stage']}-"
        f"r{plan_row['repetition']}-p{plan_row['stage_position']}-"
        f"g{_float_token(plan_row['gain_db'])}-"
        f"n{_float_token(plan_row['noise_power_db'])}"
    )
    override_file.write_text(
        SUPPORT.override_text(debug_image, plan_row["oai_rng_seed"], attach_config)
    )
    BASE.compose(
        compose_file,
        override_file,
    ...
    expected_seed = f"OAI_RNGSEED={plan_row['oai_rng_seed']}"
    ...
    if not math.isclose(float(attach_noise["observed"]), ATTACH_NOISE_DB, abs_tol=1e-6):
        raise ValidationError(
            f"UE did not attach at {ATTACH_NOISE_DB:g} dB noise: {attach_noise}"
        )
    SUPPORT.channel_command(channel_helper, "set", "ploss", ATTACH_GAIN_DB)
    SUPPORT.channel_command(channel_helper, "set", "noise_power_dB", ATTACH_NOISE_DB)
    ...
    applied_gain = SUPPORT.channel_command(
        channel_helper, "set", "ploss", plan_row["gain_db"]
    )
    applied_noise = SUPPORT.channel_command(
        channel_helper, "set", "noise_power_dB", plan_row["noise_power_db"]
    )
    try:
        ...
        while time.monotonic() < usable_deadline:
            ...
            ping_pass = SUPPORT.ping_once() if attached else False
            now = time.time()
        ...
        SUPPORT.channel_command(
            channel_helper, "set", "noise_power_dB", ATTACH_NOISE_DB
        )
        SUPPORT.channel_command(channel_helper, "set", "ploss", ATTACH_GAIN_DB)
    ...
    gnb_observation_log = SUPPORT.SUPPORT.log_suffix(
        gnb_log_text, gnb_log_prefix, "gNB"
    )
    ...
    if any(
        not math.isclose(
            float(row["applied_gain_db"]), plan_row["gain_db"], abs_tol=1e-6
        )
        for row in rows
    ):
        raise ValidationError(f"{execution_id} applied gain differs from command")
    if any(
        not math.isclose(
            float(row["applied_noise_power_db"]),
            plan_row["noise_power_db"],
            abs_tol=1e-6,
        )
        for row in rows
    ):
    ...
    if critical_failure_count != 0:
        raise ValidationError(
            f"critical radio failure during {execution_id}: {failures}"
        )
    ...
    summary = {
        **plan_row,
        "execution_id": execution_id,
        "applied_gain_command_result": applied_gain,
        "applied_noise_command_result": applied_noise,
...
def make_output(root: Path) -> Path:
    ...
    output = root / f"phase3g-bounded-response-{stamp}"
...
def execute(args: argparse.Namespace) -> int:
    ...
    if args.expected_plan_sha256 != EXPECTED_PLAN_SHA256:
        raise ValidationError(
            "runtime plan checksum does not match the frozen protocol"
        )
    ...
    if image_revision != OAI_REVISION:
        raise ValidationError(
            f"corrected image revision label mismatch: {image_revision}"
        )
    ...
    if BASE.docker_inspect("{{.State.Health.Status}}", GNB_CONTAINER) != "healthy":
        raise ValidationError("gNB is not healthy before the response experiment")
    ...
    error: str | None = None
    failed_execution: dict[str, Any] | None = None
    rollback: dict[str, Any] = {"attempted": False, "passed": False}
    mutated = False
    try:
        mutated = True
        for plan_row in plan:
            failed_execution = plan_row
            ...
            print(
                ...
                f"gain={plan_row['gain_db']:g} noise={plan_row['noise_power_db']:g} "
                f"paired={len(rows)} ping={summary['ping_success_fraction']:.3f}",
                flush=True,
            )
    except (
        KeyboardInterrupt,
        ...
        if all_rows:
            write_csv(output / "phase3g_bounded_response_telemetry.csv", all_rows)
        if mutated:
    ...
    execution_state = {
        "schema_version": 1,
        "stage": "phase_3g_bounded_gain_noise_response",
        "execution_completed": error is None and len(summaries) == len(plan),
        "error": error,
        ...
        "planned_execution_count": len(plan),
    ...
    if error is not None:
        ...
        return 1
    print(
        f"TELEMETRY={output / 'phase3g_bounded_response_telemetry.csv'}",
        flush=True,
    )
    return 0
...
def parser() -> argparse.ArgumentParser:
    ...
    root.add_argument(
        "--execution-plan",
        default="/local/repository/etc/phase3g-bounded-response-plan.csv",
    )
    root.add_argument(
        "--override-file", default="/local/upv-phase3g-response-v1/ue.override.yaml"
    )
    root.add_argument(
        "--output-root", default="/local/logs/upv-phase3g-bounded-response-v1"
    )
